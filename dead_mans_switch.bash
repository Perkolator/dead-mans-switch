...
Dead Man's Switch v1.0 | Copyright (c) 2020 Perkolator
...
Meant to be run from Synology Task Scheduler.
...
then
 	func_HELP "Script was not run from Synology Task Scheduler!" \
...
if [[ "$( synoshare --help | grep -c \
		'^[[:space:]]*--enc_unmount sharename1 sharename2 \.\.\.[[:space:]]*$' )" \
		!= "1" ]]
then
	func_ERROR "The '--enc_unmount' option of 'synoshare' program has been changed!"
fi

# Check if the "--enum" option of "synoshare" has been changed.
if [[ "$( synoshare --help | grep -c \
		'^[[:space:]]*--enum {ALL}|{LOCAL|USB|SATA|ENC|DEC|GLUSTER}{+}[[:space:]]*$' )" \
		!= "1" ]]
then
...
int_LISTEDLINENRO="$( synoshare --enum DEC \
						| grep -n '^[0-9]\+ Listed:[[:space:]]*$' \
						| cut -f1 -d: )"
if [[ "$int_LISTEDLINENRO" == "" ]]; then
	func_ERROR "Output of 'synoshare --enum' has been changed!"
fi


# Check if the "Connection" DB file can't be read.
if [[ ! -r "$str_SYNOCONNDB" ]]; then
...
if [[ "$mix_SQLTEST" == "" ]]; then
	func_ERROR "Connection DB doesn't have any share access entries!" \
				"Synology might have changed the log message syntax," \
				"or your NAS is so new that there's no connections to shares yet."
fi
...
while [[ "$1" != "" ]]; do
	...
	case "$1" in
		...
		-d | --days )	# Check for a valid option argument.
						if [[ ! "$2" =~ ^[0-9]+$ ]]; then
		...
		-l | --log )	# Check for a valid option argument.
						if [[ "$2" != [0-2] ]]; then
							func_USAGE "Argument for option '$1' is missing or invalid."
						fi
...
while IFS=$'\n' read -r str_LINE; do
	...
	# Check if the read line has a real share name.
	# Synology might change the "synoshare" program in some way.
...
done < <( synoshare --enum DEC | tail -n +"$(( $int_LISTEDLINENRO + 1 ))" )
...
if [[ "$int_LOG" == "1" && "$str_UNMOUNTLOG" != "" ]]; then
